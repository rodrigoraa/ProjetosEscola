<?php
// Proteção de LOGIN (Qualquer funcionário pode editar)
...
// --- PARTE 1: PROCESSAR O 'UPDATE' (SE O FORMULÁRIO FOR ENVIADO) ---
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $mensagem = '<p class="success-message">Aluno atualizado com sucesso! A redirecionar...</p>';
        header("Refresh: 2; url=gerir_alunos.php");

    } catch (PDOException $e) {
        $mensagem = '<p class="error-message">Erro ao atualizar aluno: ' . $e->getMessage() . '</p>';
    }
}

// --- PARTE 2: BUSCAR DADOS PARA PREENCHER O FORMULÁRIO (GET) ---
if ($id_aluno_para_editar) {
    try {
        // 1. Busca os dados DESTE Aluno
        ...
        // 2. Busca a lista de TODAS as Turmas
    ...
    header('Location: gerir_alunos.php'); // Se não há ID, volta
    exit();
}
?>
...
<body>
    ...
    <nav>
        ...
        <a href="gerir_alunos.php">Voltar para Gestão de Alunos</a>
        ...
        <form action="editar_aluno.php?id=<?php echo $aluno_data['id']; ?>" method="POST" class="sistema">
            ...
            <div>
                ...
                <input type="text" id="nome" name="nome_completo" 
                       value="<?php echo htmlspecialchars($aluno_data['nome_completo']); ?>" required>
            </div>
            
            <div>
                <label for="data_nasc">Data de Nascimento:</label>
                <input type="date" id="data_nasc" name="data_nascimento" 
                       value="<?php echo htmlspecialchars($aluno_data['data_nascimento']); ?>" required>
            </div>
                ...
                <select id="turma" name="id_turma" required>
                    ...
                    <?php foreach ($lista_turmas as $turma): ?>
                        <option value="<?php echo $turma['id']; ?>" 
                                <?php echo ($turma['id'] == $aluno_data['id_turma']) ? 'selected' : ''; ?>>
            ...
            <div>
                ...
                <a href="gerir_alunos.php" class="cancelar">Cancelar</a>
            </div>
        </form>
        <?php endif; ?>
    </main>
